...
| `OGA_ALLOWED_DEV_ORIGINS` | **unset** | Opt **in** to blocking cross-origin requests for the dev server's `/_next/*` assets. Leave it unset for normal development: defining it switches Next from `warn` to `block`, and the cross-site subresource branch ignores this list, so any value makes the app's own JavaScript 403 — and the page render blank — inside a cross-site preview pane. Only set it if you want that blocking deliberately. |
...
| `⚠ Cross origin request detected from <host> to /_next/* resource` (dev only) | The dev server is reached through a proxy/tunnel host | **Ignorable** — the assets are still served and only advice about a future Next major version is logged. Do **not** "fix" it by setting `OGA_ALLOWED_DEV_ORIGINS`: that switches Next to blocking mode and returns 403 for the app's JavaScript, blanking the page in embedded previews |
| Page loads blank / no styling when embedded in a preview pane or IDE browser, while `curl` returns 200 | Something made the dev server block cross-site `/_next/*` requests — almost always a value set in `OGA_ALLOWED_DEV_ORIGINS` | Unset `OGA_ALLOWED_DEV_ORIGINS`, restart, and confirm `curl -H 'sec-fetch-mode: no-cors' -H 'sec-fetch-site: cross-site' localhost:3000/_next/static/chunks/main-app.js` returns 200, not 403 |
